...
def main():
    ...
    pd_reader=pd.read_csv("/home/wrc/attack_on_prompt_learning/twitter_sentiment/TextFooler-master/data/train.csv")
    ...
    p_seed_label={}
    n_seed_label={}
    avg_p_vec=torch.zeros(30522)
    avg_n_vec=torch.zeros(30522)
    ...
    for idx, (ids, att, tpe, y) in enumerate(train_dataset):
        ...
        predicted_test = torch.max(pred_mask.data, 1)[1]
        pred_mask=pred_mask.cpu().detach().numpy()
        y=y.to("cpu").detach().numpy()
        torch.cuda.empty_cache()
        avg_p_vec+=np.sum(pred_mask*(1-y.reshape(-1,1)),axis=-2)
        avg_n_vec+=np.sum(pred_mask*(y.reshape(-1,1)),axis=-2)
        # pred_idx = torch.max(pred_mask.data, 1)[1]
        # for i in range(len(pred_idx)):
        #     try:
        #         label_hist[y[i]][int(pred_idx[i])]+=1
        #     except:
        #         label_hist[y[i]][int(pred_idx[i])]=1
    # k=5
    # a=sorted(p_seed_label.items(), key=lambda item:item[1],reverse=True)
    # p_seed_label=list(zip(*a))[0][:6]
    # b=sorted(n_seed_label.items(), key=lambda item:item[1],reverse=True)
    # n_seed_label=list(zip(*b))[0][:6]
    topk=10
    avg_n_vec1=avg_n_vec-avg_p_vec
    avg_p_vec1=avg_p_vec-avg_n_vec
    n_seed_label= np.argsort(avg_n_vec1)[-topk:]
    p_seed_label= np.argsort(avg_p_vec1)[-topk:]
